...
struct device_config mychardev_config[] =
{
	[MYCHARDEVA1X] = {.config_item1 = 60, .config_item2 = 20},
	[MYCHARDEVB1X] = {.config_item1 = 40, .config_item2 = 25},
	[MYCHARDEVC1X] = {.config_item1 = 30, .config_item2 = 35},
	[MYCHARDEVD1X] = {.config_item1 = 20, .config_item2 = 45} /*store this id in driver_data of struct platform_device_id*/
};

struct platform_device_id mychardev_ids[] = 
{
	[0] = {.name = "mychardev-A1x", .driver_data = MYCHARDEVA1X},	/*this matches our driver in mychar_platform_device_setup.c*/
	[1] = {.name = "mychardev-B1x", .driver_data = MYCHARDEVB1X},
	[2] = {.name = "mychardev-C1x", .driver_data = MYCHARDEVC1X},
	[3] = {.name = "mychardev-D1x", .driver_data = MYCHARDEVD1X}
};
...
int mychar_platform_driver_probe(struct platform_device *mychardev)
{
	...
	pr_info("Config item1 = %d\n", mychardev_config[mychardev->id_entry->driver_data].config_item1);
	pr_info("Config item2 = %d\n", mychardev_config[mychardev->id_entry->driver_data].config_item2);
...
struct platform_driver platform_mychardrv = 
{
	...
	.id_table = mychardev_ids,	/*for id matching driver with device*/
	.driver = {
		.name = "mychar_device" /*for name matching driver with device, this is modified */
	}
};
...
/*single macro to replace init and exit that does only register and unregister*/
/*module_platform_driver(platform_mychardrv);*/
